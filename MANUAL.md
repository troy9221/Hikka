...
Если на аккаунте включена облачная пароль (2FA), после ввода кода Hikka запросит:
...
Hikka подтянет изменения из репозитория `https://github.com/troy9221/Hikka` (настраивается через `GIT_ORIGIN_URL`) и перезапустится.
...
1. Проверьте, что процесс запущен: в терминале должно быть `Hikka started!`.
