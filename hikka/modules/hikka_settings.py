...
class HikkaSettingsMod(loader.Module):
    ...
    async def inline__setting(self, call: InlineCall, key: str, state: bool = False):
        ...
        # Re-read the custom emojis flag from config after any callable
        # setting is applied. This is cheap and keeps the runtime flag in
        # sync with the stored value without fragile name-based checks.
        ...
        # Keep the web debugger in sync with the `debugger` flag stored in DB.
        # This is what actually makes enabling/disabling debug mode work at
        # runtime, without requiring a full restart.
        await self._sync_web_debugger()
    ...
    async def _sync_web_debugger(self) -> None:
        """Synchronize the web debugger state with the stored `debugger` flag.

        Creates a :class:`WebDebugger` when the flag is enabled and there is
        no active one, and shuts it down when the flag is disabled. This makes
        the toggle in the inline settings actually take effect at runtime.
        """
        handler = logging.getLogger().handlers[0]
        debugger_enabled = self._db.get(log.__name__, "debugger", False)

        if debugger_enabled and not handler.web_debugger:
            handler.web_debugger = WebDebugger()
        elif not debugger_enabled and handler.web_debugger:
            # `shutdown()` blocks until the werkzeug server stops, so it must
            # not be called from the event loop thread. Run it in a worker
            # thread to avoid freezing the userbot.
            debugger = handler.web_debugger
            handler.web_debugger = None

            def _shutdown():
                try:
                    debugger._controller.shutdown()
                except Exception:
                    logger.debug("Can't shut down web debugger", exc_info=True)
